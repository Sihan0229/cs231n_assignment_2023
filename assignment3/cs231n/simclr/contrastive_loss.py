...
def sim_positive_pairs(out_left, out_right):
    ...
    # / L2 norms to normalize,dim = 1 means the rows
    ...
    print("\nNormalized out_right:")
    print(norm_right)


    print("\nNormalized Dot Products (pos_pairs):")
    print(pos_pairs)
...
def compute_sim_matrix(out):
    ...
    sim_matrix = norm_out @ norm_out.T # matrix multiplication
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # 使用binary mask将k = i的地方置零.
    # 创建了一个大小为 2 * N x 2 * N 的单位矩阵，表示对角线上的元素为 1，其他位置为 0
    # 通过从全1矩阵 (torch.ones_like(exponential, device=device)) 中减去这个单位矩阵
    # 将对角线上的元素置零，得到的 mask 用于在后续步骤中将指数项对角线上的值排除。
