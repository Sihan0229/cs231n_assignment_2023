...
class CaptioningTransformer(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim, wordvec_dim, num_heads=4,
        ...
        vocab_size = len(word_to_idx) # 词汇表的大小，即词汇表中唯一单词的数量。
        ...
        # 线性层 将输入的图像特征从input_dim维度投影到wordvec_dim维度。
        self.visual_projection = nn.Linear(input_dim, wordvec_dim)
        # 嵌入层 将词汇表中的单词索引映射为wordvec_dim维度的词向量。
    ...
    def forward(self, features, captions):
        ...
        decoder_out = self.transformer(tgt=captions, memory=features, tgt_mask=tgt_mask) # (N, T, W)
