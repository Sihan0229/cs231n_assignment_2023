...
class CaptioningRNN:
    ...
    def __init__(
        self,
        word_to_idx,     # 一个词汇表的字典，将每个单词映射到唯一的整数
        ...
        # 通过遍历word_to_idx字典的键值对，并将键和值进行交换来创建
    ...
    def loss(self, features, captions):
        ...
        captions_in = captions[:, :-1] #除了最后一列
        captions_out = captions[:, 1:] #除了第一列

        # 生成一个用来忽略<NULL>的布尔数组作为掩码
        # True表示该处不为<NULL>
        ...
        # 使用仿射变换（affine）从图像特征计算初始隐藏状态h0，得到形状为（N，H）的数组
        h0, cache_affine = affine_forward(features, W_proj, b_proj)
        # (2) Use a word embedding layer to transform the words in captions_in     #
        #     from indices to vectors, giving an array of shape (N, T, W).         #
        # 使用词嵌入层（word embedding）将captions_in中的单词从索引转换为向量，得到形状为（N，T，W）的数组。
    ...
    def sample(self, features, max_length=30):
        ...
        token.
        在每个时间步，我们嵌入当前单词，将当前单词和先前的隐藏状态传递给RNN，以获得下一个
        隐藏状态，使用该隐藏状态获得所有单词的分数，并选择得分最高的单词作为下一个单词。
        初始隐藏状态是通过对输入图像特征应用仿射变换来计算的，初始单词是<Start>标记。

        For LSTMs you will also have to keep track of the cell state; in that case
        the initial cell state should be zero.
        对于LSTM，您还必须跟踪单元状态；在这种情况下，初始单元状态应该为零。
        ...
        next_c = np.zeros((N, W_proj.shape[1])) #LSTM用
        # 创建一个形状为(N,)的数组word，并将其所有元素设置为起始标记<START>的索引值。
